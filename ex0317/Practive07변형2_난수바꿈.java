...
import java.util.Random; //난수 생성 유틸 호출

public class Practive07변형2_난수바꿈 {
	...
	public static void main(String[] args) {
		...
		while(s_count<10) {
			...
			int x = rand.nextInt(16); //0이상 16미만의 랜덤 정수값 생성
			for(int j=0;j<s_count;j++) { //j<i값으로 설정해놓으면 0값이 나와도 중복체크에서 벗어나 버리는 문제점이 있었음 i를 s_count로 바꿔 해결
				if (x==sort[j]) { 
					jb = true;	
					break;
				}
